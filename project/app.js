  ...
  axios.get('http://localhost:5050/devlopers').then((res) => {
    ...
    res.data.map((dev, index) => {
      data_list += `
                    ...
                    <td>
                        <a data-bs-toggle="modal" class="btn btn-info btn-sm" onclick="viewDevs(${
                          dev.id
                        })" href="#modal_view"><i class="fa-solid fa-eye-slash"></i></a>
                        <a data-bs-toggle="modal" class="btn btn-primary btn-sm" onclick="editDevs(${
                          dev.id
                        })" href="#modal_edit"><i class="fa-solid fa-edit"></i></a>
                        <a data-bs-toggle="modal" class="btn btn-danger btn-sm" onclick="deleteDevs(${
                          dev.id
                        })" href="#modal_delete"><i class="fa-solid fa-trash-can"></i></a>
                    </td>
                </tr>
            `;
    });
...
add_devs.addEventListener('submit', function (e) {
  ...
  if (
    ...
    axios
      .post('http://localhost:5050/devlopers', {
        id: '',
        name: name.value,
        email: email.value,
        cell: cell.value,
...
modal_edit.addEventListener('submit', function (e) {
  ...
  axios
    .patch(`http://localhost:5050/devlopers/${edit_id.value}`, {
      id: '',
      name: name.value,
      email: email.value,
      cell: cell.value,
